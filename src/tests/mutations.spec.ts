...
describe('mutations', () => {
  it('should add new notifiaction', () => {
    for (let notification in customNotifications) {
      store.commit('ADD_NOTIFICATION', customNotifications[notification])
    }
  ...
  it.each([1, 2])('should toggle notification as read for id %d', (id: number) => {
    const { notifications } = state
    ...
    store.commit('TOGGLE_IS_READ', notification.id)
  ...
  it.each([1,2])('should remove notification with id %d', (id: number) => {
    ...
    store.commit('REMOVE_NOTIFICATION', { id, index: 0 })
